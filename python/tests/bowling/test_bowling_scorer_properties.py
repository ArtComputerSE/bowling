...
from bowling.traditionalparser import TraditionalParser
...
def test_score_for_simple_frames_is_sum(frame):
    score = BowlingScorer(TraditionalParser()).score(frame)
...
def test_score_between_zero_and_300_for_full_frames(frames):
    assert 0 <= BowlingScorer(TraditionalParser()).score(frames) <= 300
